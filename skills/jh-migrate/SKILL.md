...
**FLAGS**: `import-pages` supports `--batch-size`, `--category-file`, `--product-file`, `--rewriteurl-file`, `--notification-email`.
...
### Step 3.9.5: Build Sidebar Navigation Menu (from SOURCE_SIDEBAR_NAV)

Write a PHP script that:
1. Creates or clears 'Sidebar Navigation' menu
2. For each top-level item in `SOURCE_SIDEBAR_NAV`:
   - If it matches a product category name (e.g., "Spring Planted Bulbs") → add as `taxonomy` item + auto-add all subcategories as children
   - If it matches a page slug → add as `post_type` item
   - If it's "Home" → add as custom link to `home_url('/')`
   - If it's "Search" → add as custom link to `home_url('/?s=')`
3. Assigns as widget to sidebar-1
...
### Step 3.9.6: Add Sidebar Widgets (from SOURCE_SIDEBAR_WIDGETS)

For each widget detected in Phase 1.5.3, create the appropriate widget:
...
Use Playwright browser to verify the migrated site renders correctly:

1. Navigate to the migrated site homepage
2. Take an accessibility snapshot
3. Verify these elements are present:
   - Header menu with correct links
   - Left/right sidebar with navigation + widgets
   - Homepage content
   - Footer menu with correct links + copyright text

4. If `SOURCE_SITE_URL` is available, also navigate to the source site and take a side-by-side screenshot for comparison

5. Report any visual discrepancies found:
...
|--------------------|-----------------------|-----------------------|--------|
| Header links       | {source_count} items  | {migrated_count}      | OK/MISMATCH |
| Sidebar position   | {left/right}          | {detected}            | OK/MISMATCH |
| Sidebar nav items  | {source_count}        | {migrated_count}      | OK/MISMATCH |
| Testimonial widget | Present               | {present/missing}     | OK/MISMATCH |
| Newsletter form    | Present               | {present/missing}     | OK/MISMATCH |
| Footer links       | {source_count} items  | {migrated_count}      | OK/MISMATCH |
| Copyright text     | Present               | {present/missing}     | OK/MISMATCH |
| Homepage content   | "{title}"             | "{detected_title}"    | OK/MISMATCH |
```

If any MISMATCH is found, report the issue and suggest a fix but do NOT stop the migration.
